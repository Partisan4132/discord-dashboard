...
function renderServerPicker() {
  ...
  menu.innerHTML = state.servers.length
    ? state.servers
        .map(server => {
      ...
      <span class="server-option-icon">${server.icon ? `<img src="https://cdn.discordapp.com/icons/${escapeHtml(server.id )}/${escapeHtml(server.icon)}.png?size=64" alt="" />` : escapeHtml(server.name.slice(0, 1).toUpperCase())}</span>
      <span class="server-option-copy"><strong>${escapeHtml(server.name)}</strong><small>${server.botInstalled ? `${escapeHtml(server.accessRole || "admin")} · Installed` : "Bot not installed"}</small></span>
      ${server.botInstalled ? "<span class=\"server-check\">✓</span>" : `<a class="server-invite" href="${escapeHtml(server.inviteUrl || "#")}" target="_blank" rel="noopener">Invite bot</a>`}
        ...
        .join("")
    : '<div class="empty-state">No administrator-accessible servers found.</div>';

  if ($("#serverPickerButton")) {
    $("#serverPickerButton").setAttribute(
      "aria-expanded",
...
async function loadServers() {
  ...
  const available =
    state.servers.find(
      server => server.id === state.guildId && server.botInstalled
    ) ||
    state.servers.find(server => server.id === result.selectedGuildId) ||
    state.servers.find(server => server.botInstalled) ||
    state.servers[0];
...
async function switchServer(guildId) {
  ...
  if (!server.botInstalled) {
    if (server.inviteUrl) {
      window.open(server.inviteUrl, "_blank", "noopener");
    }
    return;
  }
...
function showSection(section) {
  ...
  if (state.user) {
    loadSection(section).catch(showAuthError);
  }
}
...
function renderActivity(items = []) {
  ...
  $("#activityFeed").innerHTML = items.length
    ? items
        .slice(0, 12)
        .map(
          item => `<div class="activity-row"><div><strong>${escapeHtml(item.action || item.type || "Activity")}</strong><small>${escapeHtml(item.target || item.username || "Server event")}</small></div><small>${formatDate(item.at || item.createdAt)}</small></div>`
        )
        .join("")
...
function formatDuration(seconds) {
  ...
  return (
    [hours && `${hours}h`, minutes && `${minutes}m`, secs && `${secs}s`]
      .filter(Boolean)
      .join(" ") || "0s"
  );
}
  ...
  $("#applicationsList").innerHTML = pending.length
    ? pending
        .map(row => {
          ...
          const applicant =
            row.username || row.user?.username || row.userId || "Unknown applicant";

          return `<article class="application-card" data-application-id="${escapeHtml(row.id)}"><div class="application-card-head"><div><p class="eyebrow">${escapeHtml(row.typeName || row.typeId || "Application")} application submitted</p><h3>${escapeHtml(applicant)}'s application</h3><p class="application-user">User: ${escapeHtml(row.userMention || row.mention || row.userId || "—")}</p></div><span class="status">Pending</span></div><div class="answer-list">${answers.length ? answers.map((item, index) => `<div class="answer-item"><div class="answer-question">${index + 1}. ${escapeHtml(item.question || item.label || `Question ${index + 1}`)}</div><div class="answer-value">${escapeHtml(item.answer || "(no answer)")}</div></div>`).join("") : '<div class="empty-state">No answers were recorded.</div>'}</div><div class="submission-stats"><p class="eyebrow">Submission stats</p><div class="stats-grid"><div><span>User ID</span><strong>${escapeHtml(row.userId || "—")}</strong></div><div><span>Username</span><strong>${escapeHtml(applicant)}</strong></div><div><span>Duration</span><strong>${escapeHtml(formatDuration(row.durationSeconds ?? stats.durationSeconds))}</strong></div><div><span>Joined guild</span><strong>${escapeHtml(relativeDate(row.joinedAt ?? stats.joinedAt))}</strong></div><div><span>Submitted</span><strong>${escapeHtml(formatDate(row.submittedAt ?? row.createdAt))}</strong></div></div></div><div class="application-actions"><button class="primary application-action" data-id="${escapeHtml(row.id)}" data-decision="approved">Accept application</button><button class="danger application-action" data-id="${escapeHtml(row.id)}" data-decision="denied">Deny application</button></div></article>`;
        })
        .join("")
...
function options(items = [], selected, emptyLabel) {
  return `<option value="">${escapeHtml(emptyLabel)}</option>${items
    .map(
      item => `<option value="${escapeHtml(item.id)}" ${item.id === selected ? "selected" : ""}>${escapeHtml(item.name)}</option>`
    )
    .join("")}`;
}
...
function selectedType() {
  return (
    (state.settings?.applicationTypes || []).find(
...
function renderApplicationList() {
  ...
  $("#applicationTypeList").innerHTML = types.length
    ? types
        .map(
          type => `<button type="button" class="resource-item ${type.id === state.selectedTypeId ? "selected" : ""}" data-type-id="${escapeHtml(type.id)}"><span class="resource-icon">${escapeHtml(type.emoji || "▤")}</span><span><strong>${escapeHtml(type.name)}</strong><small>${type.enabled !== false ? "Enabled" : "Disabled"} · ${(type.questions || []).length} questions</small></span></button>`
        )
        .join("")
...
function renderApplicationEditor() {
  ...
  if ($("#selectedApplicationReviewerRole")) {
  ...
  if ($("#selectedApplicationAcceptedRole")) {
  ...
  if ($("#selectedApplicationReviewChannel")) {
...
function saveEditorToState() {
  ...
  if ($("#selectedApplicationQuestions")) {
    type.questions = $$(
      ".question-row",
      $("#selectedApplicationQuestions")
    ).map((row, index) => ({
      id: type.questions?.[index]?.id || crypto.randomUUID(),
      label:
        row.querySelector('[data-q-field="label"]')?.value || "Question",
      maxLength: Number(
        row.querySelector('[data-q-field="maxLength"]')?.value || 1200
      ),
      required:
        row.querySelector('[data-q-field="required"]')?.checked !== false
    }));
  }
}
...
function fillPanelFields() {
  ...
  [
    ...
    "applicationPanelDescription",
    "applicationPanelColor",
    "applicationPanelImageUrl",
    "applicationPanelPlaceholder",
    "applicationPanelInteraction"
  ].forEach(id => {
    if ($(`#${id}`)) $(`#${id}`).value = settings[id] || "";
  });

  if ($("#applicationPanelDeleteOld")) {
    $("#applicationPanelDeleteOld").checked =
      settings.applicationPanelDeleteOld !== false;
  }

  if ($("#panelApplicationChecklist")) {
    $("#panelApplicationChecklist").innerHTML =
      (settings.applicationTypes || [])
        .map(
          type => `<label class="panel-check"><input type="checkbox" data-panel-type="${escapeHtml(type.id)}" ${type.enabled !== false ? "checked" : ""} /><span><strong>${escapeHtml(type.name)}</strong><small>${escapeHtml(type.description || "Application form")}</small></span></label>`
        )
        .join("") || '<div class="empty-state">Create an application first.</div>';
  }
}
...
function collectSettings() {
  ...
  return {
    ...state.settings,
    applicationTypes: (state.settings.applicationTypes || []).map(type => ({
      ...type,
      enabled: enabled.size ? enabled.has(type.id) : type.enabled !== false
    })),
    ...
    applicationReviewedChannelId:
      $("#applicationReviewedChannelId")?.value || "",
    applicationReviewerRoleId:
      $("#applicationReviewerRoleId")?.value || "",
    applicationPanelTitle:
      $("#applicationPanelTitle")?.value.trim() || "",
    applicationPanelDescription:
      $("#applicationPanelDescription")?.value.trim() || "",
    applicationPanelColor:
      $("#applicationPanelColor")?.value.trim() || "#2bd9fe",
    applicationPanelImageUrl:
      $("#applicationPanelImageUrl")?.value.trim() || "",
    applicationPanelPlaceholder:
      $("#applicationPanelPlaceholder")?.value.trim() ||
      "Choose an application type",
    applicationPanelInteraction:
      $("#applicationPanelInteraction")?.value || "dropdown",
    applicationPanelDeleteOld:
...
function renderExtraPanels() {
  ...
  $("#extraPanelsList").innerHTML = state.extraPanels.length
    ? state.extraPanels
        .map(
          panel => `<button type="button" class="resource-item ${panel.id === state.selectedExtraPanelId ? "selected" : ""}" data-extra-panel-id="${escapeHtml(panel.id)}"><span class="resource-icon">▣</span><span><strong>${escapeHtml(panel.name)}</strong><small>${panel.messageId ? "Published" : "Not published"}</small></span></button>`
        )
        .join("")
    : '<div class="empty-state">No additional panels yet.</div>';
}

function renderExtraPanelForm() {
  const panel = state.extraPanels.find(
    item => item.id === state.selectedExtraPanelId
  );

  if (!$("#extraPanelForm")) return;

  if (!panel) {
    $("#extraPanelForm").classList.add("hidden");
    return;
  }
  ...
  set("extraPanelName", panel.name);
  set("extraPanelTitle", panel.title);
  set("extraPanelDescription", panel.description);
  set("extraPanelColor", panel.color);
  set("extraPanelImageUrl", panel.imageUrl);
  set("extraPanelPlaceholder", panel.placeholder);

  if ($("#extraPanelChannelId")) {
    $("#extraPanelChannelId").innerHTML = channelOptions(
      panel.channelId,
      "Choose panel channel"
    );
    $("#extraPanelChannelId").value = panel.channelId || "";
  }

  if ($("#extraPanelDeleteOld")) {
    $("#extraPanelDeleteOld").checked = panel.deleteOld !== false;
  }

  if ($("#extraPanelTypes")) {
    $("#extraPanelTypes").innerHTML =
      (state.settings?.applicationTypes || [])
        .map(
          type => `<label class="panel-check"><input type="checkbox" data-extra-type-id="${escapeHtml(type.id)}" ${panel.applicationTypeIds.includes(type.id) ? "checked" : ""} /><span><strong>${escapeHtml(type.name)}</strong><small>${escapeHtml(type.description || "Application form")}</small></span></label>`
        )
        .join("") || '<div class="empty-state">Create an application first.</div>';
  }
}
...
function readExtraPanelForm() {
  return {
    name: $("#extraPanelName")?.value.trim() || "New panel",
    channelId: $("#extraPanelChannelId")?.value || "",
    title: $("#extraPanelTitle")?.value.trim() || "Applications",
    description: $("#extraPanelDescription")?.value.trim() || "",
    color: $("#extraPanelColor")?.value.trim() || "#2bd9fe",
    imageUrl: $("#extraPanelImageUrl")?.value.trim() || "",
    placeholder:
      $("#extraPanelPlaceholder")?.value.trim() ||
      "Choose an application type",
    deleteOld: $("#extraPanelDeleteOld")?.checked !== false,
    applicationTypeIds: $$('[data-extra-type-id]:checked').map(
      input => input.dataset.extraTypeId
    )
  };
}
...
function renderAccess() {
  ...
  if ($("#settingsRoleSummary")) {
    $("#settingsRoleSummary").textContent =
      access.role === "owner" ? "Owner access" : "Administrator access";
  }

  if ($("#settingsAdminsList")) {
    $("#settingsAdminsList").innerHTML = access.members?.length
      ? access.members
          .map(
            member => `<div class="admin-row"><div><strong>${escapeHtml(member.username)}</strong><small>${escapeHtml(member.role)} · ${escapeHtml(member.userId)}</small></div><button class="danger remove-admin" type="button" data-admin-id="${escapeHtml(member.id)}">Remove</button></div>`
          )
          .join("")
      : '<div class="empty-state">No manually added dashboard members.</div>';
  }

  const permissions = access.permissions || {};
  $$('[data-permission-key]').forEach(input => {
    input.checked = permissions[input.dataset.permissionKey] === true;
    input.disabled = access.role !== "owner";
  });
...
async function completeOAuthHandoff() {
  const token =
    new URLSearchParams(window.location.hash.replace(/^#/, "")).get("oauth") ||
...
bindAll('[data-action="refresh"]', "click", () =>
  refreshDashboard().catch(showAuthError)
);
...
bind("#serverPickerMenu", "click", event => {
  ...
  if (option) {
    switchServer(option.dataset.serverId).catch(showAuthError);
  }
});

document.addEventListener("click", event => {
  if (!event.target.closest(".sidebar-server")) {
    toggleServerMenu(false);
  }
});
...
bind("#loginButton", "click", async () => {
  if (!state.user) {
    return window.location.assign(`${API}/auth/discord`);
  }
...
bind("#selectedApplicationQuestions", "click", event => {
  ...
  saveEditorToState();
  type.questions.splice(
    Number(button.closest(".question-row").dataset.questionIndex),
    1
  );
  renderApplicationEditor();
});
...
bind("#applicationsList", "click", async event => {
  ...
  try {
    await api(
      `/api/applications/${button.dataset.id}/${button.dataset.decision}`,
      { method: "POST" }
    );
  ...
  try {
    const result = await api("/api/panels", {
      ...
      body: JSON.stringify({
        ...
        deleteOld: true,
        applicationTypeIds: (state.settings?.applicationTypes || [])
...
bind("#saveExtraPanel", "click", async () => {
  ...
  try {
    const result = await api(
      `/api/panels/${state.selectedExtraPanelId}`,
      {
        method: "PUT",
        body: JSON.stringify(readExtraPanelForm())
      }
    );
...
bind("#publishExtraPanel", "click", async () => {
  ...
  try {
    await api(`/api/panels/${state.selectedExtraPanelId}`, {
      method: "PUT",
      body: JSON.stringify(readExtraPanelForm())
    });

    const result = await api(
      `/api/panels/${state.selectedExtraPanelId}/publish`,
      { method: "POST" }
    );
...
bind("#deleteExtraPanel", "click", async () => {
  if (
    !state.selectedExtraPanelId ||
    !confirm("Delete this additional panel?")
  ) {
    return;
  }
...
bind("#extraPanelsList", "dblclick", event => {
  const item = event.target.closest("[data-extra-panel-id]");
  if (!item) return;
...
bind("#addSettingsAdmin", "click", async () => {
  try {
    const result = await api("/api/access/members", {
      method: "POST",
      body: JSON.stringify({
        username: $("#settingsAdminUsername")?.value.trim() || "",
        role: $("#settingsAdminRole")?.value || "admin"
      })
    });

    state.access.members = result.members;
    if ($("#settingsAdminUsername")) {
...
bind("#settingsAdminsList", "click", async event => {
  ...
  try {
    const result = await api(
      `/api/access/members/${button.dataset.adminId}`,
      { method: "DELETE" }
    );
...
bind("#saveAdminPermissions", "click", async () => {
  try {
    const permissions = Object.fromEntries(
      $$('[data-permission-key]').map(input => [
        input.dataset.permissionKey,
        input.checked
      ])
    );

    const result = await api("/api/access/permissions", {
      method: "PUT",
