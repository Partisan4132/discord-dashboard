...
function formatRelativeDate(value) { if (!value) return "—"; const date = new Date(value); if (Number.isNaN(date.getTime())) return escapeHtml(value); const days = Math.max(0, Math.floor((Date.now() - date.getTime()) / 86400000)); if (!days) return "today"; if (days === 1) return "1 day ago"; if (days < 30) return `${days} days ago`; const months = Math.floor(days / 30); return `${months} month${months === 1 ? "" : "s"} ago`; }
function applicationStats(row) { const stats = row.submissionStats || row.stats || {}; return { duration: formatDuration(row.durationSeconds ?? row.duration ?? stats.durationSeconds ?? stats.duration), joined: formatRelativeDate(row.joinedAt ?? row.guildJoinedAt ?? stats.joinedAt ?? stats.guildJoinedAt), submitted: formatDate(row.submittedAt ?? row.createdAt) }; }

function renderApplications(rows = []) {
  const pending = rows.filter(row => row.status === "pending");
  $("#applicationsList").innerHTML = pending.length ? pending.map(row => {
    const stats = applicationStats(row), answers = Array.isArray(row.answers) ? row.answers : [], applicant = row.username || row.user?.username || row.userId || "Unknown applicant", displayUser = row.userMention || row.mention || `<@${escapeHtml(row.userId || "")}>`;
    return `<article class="application-card" data-application-id="${escapeHtml(row.id)}"><div class="application-card-head"><div><p class="eyebrow">${escapeHtml(row.typeName || row.typeId || "Application")} application submitted</p><h3>${escapeHtml(applicant)}'s “${escapeHtml(row.typeName || row.typeId || "Application")}” Application Submitted</h3><p class="application-user">User: ${escapeHtml(displayUser)}</p></div><span class="status">Pending</span></div><div class="answer-list">${answers.length ? answers.map((item, index) => `<div class="answer-item"><div class="answer-question">${index + 1}. ${escapeHtml(item.question || item.label || `Question ${index + 1}`)}</div><div class="answer-value">${escapeHtml(item.answer || "(no answer)").replace(/\n/g, "  
")}</div></div>`).join("") : '<div class="empty-state">No answers were recorded.</div>'}</div><div class="submission-stats"><p class="eyebrow">Submission stats</p><div class="stats-grid"><div><span>User ID</span><strong>${escapeHtml(row.userId || "—")}</strong></div><div><span>Username</span><strong>${escapeHtml(applicant)}</strong></div><div><span>Duration</span><strong>${escapeHtml(stats.duration)}</strong></div><div><span>Joined guild</span><strong>${escapeHtml(stats.joined)}</strong></div><div><span>Submitted</span><strong>${escapeHtml(stats.submitted)}</strong></div></div></div><div class="application-actions"><button class="primary application-action" data-id="${escapeHtml(row.id)}" data-decision="approved">Accept application</button><button class="danger application-action" data-id="${escapeHtml(row.id)}" data-decision="denied">Deny application</button></div></article>`;
  }).join("") : '<div class="empty-state">There are no pending applications.</div>';
}

function options(items, selected, emptyLabel) { return `<option value="">${escapeHtml(emptyLabel)}</option>` + items.map(item => `<option value="${escapeHtml(item.id)}" ${item.id === selected ? "selected" : ""}>${escapeHtml(item.name)}</option>`).join(""); }
...
function renderApplicationEditor() {
  ...
  if (!type) { $("#applicationForm").classList.add("hidden"); return; }
  ...
  $("#selectedApplicationQuestions").innerHTML = (type.questions || []).length ? type.questions.map((question, index) => `<div class="question-row" data-question-index="${index}"><label><span class="field-label">Question ${index + 1}</span><textarea data-q-field="label" maxlength="1000">${escapeHtml(question.label)}</textarea></label><label><span class="field-label">Max characters</span><input data-q-field="maxLength" type="number" min="20" max="2000" value="${Number(question.maxLength || 1200)}" /></label><label class="check"><input data-q-field="required" type="checkbox" ${question.required !== false ? "checked" : ""} /> Required <button type="button" class="danger remove-question">Remove</button></label></div>`).join("") : '<div class="empty-state">No questions yet. Add your first question.</div>';
}
function saveEditorToState() {
  ...
  type.questions = $$(".question-row", $("#selectedApplicationQuestions")).map((row, index) => ({ id: type.questions?.[index]?.id || crypto.randomUUID(), label: row.querySelector('[data-q-field="label"]').value.trim(), maxLength: Number(row.querySelector('[data-q-field="maxLength"]').value || 1200), required: row.querySelector('[data-q-field="required"]').checked }));
}
function renderPanelChecklist() {
  const types = state.settings?.applicationTypes || [];
  $("#panelApplicationChecklist").innerHTML = types.length ? types.map(type => `<label class="panel-check"><input type="checkbox" data-panel-type="${escapeHtml(type.id)}" ${type.enabled !== false ? "checked" : ""} /><span><strong>${escapeHtml(type.name)}</strong><small>${escapeHtml(type.description || "Application form")}</small></span></label>`).join("") : '<div class="empty-state">Create an application first.</div>';
}
function fillPanelFields() {
  ...
  ["applicationPanelChannelId", "applicationReviewChannelId", "applicationReviewedChannelId", "applicationReviewerRoleId", "applicationAcceptedRoleId"].forEach(id => $("#" + id).value = s[id] || "");
  ...
  $("#applicationPanelInteraction").value = s.applicationPanelInteraction || "dropdown";
  $("#applicationPanelDeleteOld").checked = s.applicationPanelDeleteOld !== false;
  renderPanelChecklist();
}
function collectSettings() {
  saveEditorToState();
  const enabledFromPanel = new Set($$("[data-panel-type]:checked").map(input => input.dataset.panelType));
  const applicationTypes = (state.settings.applicationTypes || []).map(type => ({ ...type, enabled: enabledFromPanel.size ? enabledFromPanel.has(type.id) : type.enabled !== false }));
  return { ...state.settings, applicationTypes, applicationPanelChannelId: $("#applicationPanelChannelId").value, applicationReviewChannelId: $("#applicationReviewChannelId").value, applicationReviewedChannelId: $("#applicationReviewedChannelId").value, applicationReviewerRoleId: $("#applicationReviewerRoleId").value, applicationAcceptedRoleId: $("#applicationAcceptedRoleId").value, applicationPanelTitle: $("#applicationPanelTitle").value.trim(), applicationPanelDescription: $("#applicationPanelDescription").value.trim(), applicationPanelColor: $("#applicationPanelColor").value.trim(), applicationPanelImageUrl: $("#applicationPanelImageUrl").value.trim(), applicationPanelInteraction: $("#applicationPanelInteraction").value, applicationPanelPlaceholder: $("#applicationPanelPlaceholder").value.trim(), applicationPanelDeleteOld: $("#applicationPanelDeleteOld").checked };
}
...
async function loadSection(section) {
  ...
  if (section === "applications" || section === "panels" || section === "welcome") { state.settings = await api("/api/settings"); state.channels = await api("/api/channels"); state.selectedTypeId = state.selectedTypeId || state.settings.applicationTypes?.[0]?.id || null; if (section === "applications") renderApplicationEditor(); if (section === "panels") fillPanelFields(); if (section === "welcome") { $("#welcomeChannelId").innerHTML = channelOptions(state.settings.welcomeChannelId, "Choose welcome channel"); $("#welcomeChannelId").value = state.settings.welcomeChannelId || ""; $("#welcomeImageUrl").value = state.settings.welcomeImageUrl || ""; } }
}
...
$("#loginButton").addEventListener("click", async () => { if (!state.user) { window.location.assign(`${API}/auth/discord`); return; } await api("/auth/logout", { method: "POST" }); state.user = null; updateConnection(false); });
$("#addTypeTop").addEventListener("click", () => { const type = { id: crypto.randomUUID(), name: "New application", description: "Start this application", emoji: "📋", enabled: true, reviewerRoleId: "", approvalRoleId: "", reviewChannelId: "", questions: [] }; state.settings.applicationTypes.push(type); state.selectedTypeId = type.id; renderApplicationEditor(); });
$("#applicationTypeList").addEventListener("click", event => { const item = event.target.closest("[data-type-id]"); if (!item) return; saveEditorToState(); state.selectedTypeId = item.dataset.typeId; renderApplicationEditor(); });
$("#addQuestionTop").addEventListener("click", () => { const type = selectedType(); if (!type) return; saveEditorToState(); type.questions.push({ id: crypto.randomUUID(), label: "New question", required: true, maxLength: 1200 }); renderApplicationEditor(); });
$("#selectedApplicationQuestions").addEventListener("click", event => { const button = event.target.closest(".remove-question"); if (!button) return; const type = selectedType(); saveEditorToState(); const row = button.closest(".question-row"); type.questions.splice(Number(row.dataset.questionIndex), 1); renderApplicationEditor(); });
$("#saveApplications").addEventListener("click", async () => { try { state.settings = await api("/api/settings", { method: "PUT", body: JSON.stringify({ ...collectSettings(), applicationTypes: state.settings.applicationTypes }) }); renderApplicationEditor(); message($("#applicationMessage"), "Application saved.", "success"); } catch (error) { message($("#applicationMessage"), error.message, "error"); } });
$("#saveSettings").addEventListener("click", async () => { try { state.settings = await api("/api/settings", { method: "PUT", body: JSON.stringify(collectSettings()) }); fillPanelFields(); message($("#builderMessage"), "Panel saved.", "success"); } catch (error) { message($("#builderMessage"), error.message, "error"); } });
$("#publishPanel").addEventListener("click", async () => { try { state.settings = await api("/api/settings", { method: "PUT", body: JSON.stringify(collectSettings()) }); const result = await api("/api/panel/publish", { method: "POST" }); state.settings = result.settings || state.settings; fillPanelFields(); message($("#builderMessage"), "Panel sent to Discord.", "success"); } catch (error) { message($("#builderMessage"), error.message, "error"); } });
